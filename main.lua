	...
	function menu.new_tab(tab_image)
		...
		TabButton.MouseButton1Down:Connect(function()
			...
			for _, TButtons in pairs(TabButtons:GetChildren()) do
				if not TButtons:IsA("TextButton") then
					continue
				end
		...
		function tab.new_section(section_name)
			...
			for _, SectionButtons in pairs(TabSections:GetChildren()) do
				if SectionButtons:IsA("UIListLayout") then
					continue
				end
			...
			function section.new_sector(sector_name, sector_side)
				...
				function sector.element(type, text, data, callback, c_flag)
					text, data, callback =
						text and text or type, data and data or {}, callback and callback or function() end
					...
					if type == "Toggle" then
						...
						function element:add_keybind(key_default, key_callback)
							...
							uis.InputBegan:Connect(function(input)
								if
									input.UserInputType == Enum.UserInputType.MouseButton1
									or input.UserInputType == Enum.UserInputType.MouseButton2 and not binding
								then
							...
							for _, TypeButton in next, KeybindFrame:GetChildren() do
								if TypeButton:IsA("UIListLayout") then
									continue
								end
								...
								TypeButton.MouseButton1Down:Connect(function()
									...
									for _, TypeButton2 in next, KeybindFrame:GetChildren() do
										if TypeButton2:IsA("UIListLayout") then
											continue
										end
										library:tween(
											TypeButton2,
							...
							uis.InputBegan:Connect(function(input)
								if is_binding then
									...
									Keybind.Size = UDim2.new(
										0,
										library:get_text_size(Keybind.Text, 14, Enum.Font.Ubuntu, Vector2.new(700, 20)).X
											+ 3,
										0,
										20
									)
								...
								if extra_value.Key ~= nil and not is_binding then
									...
									if key == extra_value.Key then
										if extra_value.Type == "Hold" then
											...
											key_callback(extra_value)
											menu.values[tab.tab_num][section_name][sector_name][extra_flag] =
												extra_value
										end
									end
								end
							...
							Keybind.MouseButton1Down:Connect(function()
								if not is_binding then
									...
									Keybind.Size = UDim2.new(
										0,
										library:get_text_size("[ ... ]", 14, Enum.Font.Ubuntu, Vector2.new(700, 20)).X
											+ 3,
										0,
										20
									)
						...
						function element:add_color(color_default, has_transparency, color_callback)
							...
							color.h = (
								math.clamp(
									HuePick.AbsolutePosition.Y - HuePicker.AbsolutePosition.Y,
									0,
									HuePicker.AbsoluteSize.Y
								) / HuePicker.AbsoluteSize.Y
							)
							color.s = 1
								- (
									math.clamp(
										ColorPick.AbsolutePosition.X - ColorPick.AbsolutePosition.X,
										0,
							...
							color.v = 1
								- (
									math.clamp(
										ColorPick.AbsolutePosition.Y - ColorPick.AbsolutePosition.Y,
										0,
							...
							function color.update_color()
								local ColorX = (
									math.clamp(mouse.X - ColorPicker.AbsolutePosition.X, 0, ColorPicker.AbsoluteSize.X)
									/ ColorPicker.AbsoluteSize.X
								)
								local ColorY = (
									math.clamp(mouse.Y - ColorPicker.AbsolutePosition.Y, 0, ColorPicker.AbsoluteSize.Y)
							...
							function color:set_value(new_value, cb)
								...
								local duplicate =
									Color3.new(extra_value.Color.R, extra_value.Color.G, extra_value.Color.B)
					...
					elseif type == "Combo" then
						...
						function element:set_value(new_value, cb)
							...
							for _, DropButton in next, DropdownScroll:GetChildren() do
								if not DropButton:IsA("TextButton") then
									continue
								end
					...
					elseif type == "TextBox" then
						...
						function element:set_visible(bool)
							if bool then
								...
								if not ButtonFrame.Visible then
									return
								end
								Border.Size = Border.Size + UDim2.new(0, 0, 0, -30)
					...
					elseif type == "Scroll" then
						...
						function element:set_value(new_value, cb)
							...
							library:tween(
								ScrollFrame[value.Scroll].ButtonText,
								TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
								{ TextColor3 = Color3.fromRGB(255, 255, 255) }
							)
					...
					elseif type == "Slider" then
						...
						SliderButton.MouseButton1Down:Connect(function()
							SliderFrame.Size =
								UDim2.new(0, math.clamp(mouse.X - SliderFrame.AbsolutePosition.X, 0, 260), 1, 0)
